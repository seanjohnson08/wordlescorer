...
T.get('statuses/show/:id', {id: '1533961937352105984', include_ext_alt_text: true}).then(({data}: any) => {
  //console.log(data)
  const parentAltText: string = data?.extended_entities?.media?.[0]?.ext_alt_text || '';
  const parentWordleResult: string[] = getWordleMatrixFromText(data.text);
...
T.get('statuses/show/:id', {id: '1608109623889756160', include_ext_alt_text: true}).then(({data}: any) => {
  const text: string = data?.extended_entities?.media?.[0]?.ext_alt_text || '';
  console.log(text);
