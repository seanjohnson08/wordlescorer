...
import Twit, { ConfigKeys, Response as TwitResponse } from 'twit';

const router: Router = express.Router();

const TWIT_CONFIG: ConfigKeys = {
...
router.get('/initphotos', function(req: Request, res: Response, next: NextFunction) {
  ...
  AnalyzedTweetsDB.read().then((data: any) => {
    ...
    const formattedData = Array.from(formattedDataSet).map((item: string) => item.slice(1));
    ...
    })).then((outputs: TwitResponse[]) => {
      const formattedOutputs = outputs.map(({data}: TwitResponse) => {
...
const defaultPhotoUrl: string = 'https://abs.twimg.com/sticky/default_profile_images/default_profile_normal.png';
