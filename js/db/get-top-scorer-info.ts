...
async function getTopScorerInfo(date: Date): Promise<Scorer | null> {
  ...
  let scorerList: Scorer[] = Object.values(data);
  ...
  if (globalStats.length > 1) {
    ...
    scorerList.forEach((scorer) => {
      ...
      scorer.aboveTotal = formatter.format(scorer.aboveTotal);
    });
  }
  /**
