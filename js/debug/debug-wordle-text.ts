...
twitterClient.get('statuses/show/:id', {id: '1608040651546836995', include_ext_alt_text: true, tweet_mode: 'extended'}).then(({data}) => {
    ...
    const wordleResult = getWordleMatrixFromText(data.text || data.full_text);
    const WordleAlt = getWordleMatrixFromImageAltText(parentAltText);
...
async function getTopScorerConsole() {
    const date = new Date('12-28-2022');
    const scorer = await getTopScorerInfo(date);
    const finalStatus = `${scorer.name}! They scored ${scorer.score} points for Wordle ${scorer.wordleNumber} and solved it on row ${scorer.solvedRow}! That's better than ${scorer.aboveTotal} (~${scorer.percentage}) other users. ${getCompliment()}`;
